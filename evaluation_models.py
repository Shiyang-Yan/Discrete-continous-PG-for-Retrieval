...
from model_rlrl_triplet_rebuttal_baseline import VSRN as VSRN1
from model import order_sim
from model_rlrl_triplet_rebuttal_baseline import VSRN as VSRN2
...
def encode_data1(model, data_loader, log_step=10, logging=print):
    ...
    img_embs = None
    cap_embs = None
    for i, (images, captions, lengths, ids, caption_labels, caption_masks, _, boxes, _, language) in enumerate(data_loader):
        # make sure val logger is used
        model.logger = val_logger

        # compute the embeddings
        img_emb, cap_emb, GCN_img_emd, _, _, _, _, _, _  = model.forward_emb(images, language, lengths,
...
def encode_data2(model, data_loader, log_step=10, logging=print):
    """Encode all images and captions loadable by `data_loader`
    """
    batch_time = AverageMeter()
    val_logger = LogCollector()
    ...
    img_embs = None
    cap_embs = None
    for i, (images, captions, lengths, ids, caption_labels, caption_masks, _, _,_,language) in enumerate(data_loader):
        # make sure val logger is used
        model.logger = val_logger
...
def evalrank(model_path, model_path2, data_path=None, split='dev', fold5=False):
    ...
    img_embs, cap_embs = encode_data1(model, data_loader)
    img_embs2, cap_embs2 = encode_data2(model2, data_loader)
...
def i2t(images, captions, images2, captions2, npts=None, measure='cosine', return_ranks=False):
    ...
    """
    if npts is None:
        npts = math.ceil(images.shape[0] / 5)
    index_list = []
    print (npts)
