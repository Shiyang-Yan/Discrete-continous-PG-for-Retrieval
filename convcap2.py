...
class convcap(nn.Module):
    ...
    def __init__(self, num_wordclass, num_layers=3, is_attention=True, nfeats=512, dropout=.1):
        ...
        self.emb_1 = Linear(300, nfeats, dropout=dropout)
        self.imgsproj = Linear(2048, self.nfeats)
    ...
    def forward(self, imgsfeats, imgsfc7, wordclass, is_attention):
        if is_attention:
            imgsfeats = F.relu(self.imgsproj(imgsfeats)).transpose(2,1)
        #imgsfeats = imgsfeats.transpose(2,1)
        attn_buffer = None
        #wordemb = self.emb_0(wordclass)
        wordemb = self.emb_1(wordclass)
