...
vectors = bcolz.open(f'glove.840B.300d/840B.300.dat')[:]
words = pickle.load(open(f'glove.840B.300d/840B.300_words.pkl', 'rb'))
word2idx = pickle.load(open(f'glove.840B.300d/840B.300_idx.pkl', 'rb'))
...
class FlickrDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        caption.extend([vocab(token) for token in tokens])
        caption.append(vocab('<end>'))
        target = torch.Tensor(caption).float()
        return image, target, index, img_id
    def __len__(self):
        return len(self.ids)
...
class PrecompDataset(data.Dataset):
    ...
    def __init__(self, data_path, data_split, vocab, opt):
        self.vocab = vocab
        vocab_list = list(vocab.idx2word.values())
        self.vocab_int = intersection(vocab_list, words)
        loc = data_path + '/'
        # Captions
        self.captions = []
        token_caption = []
        ...
        # Image features
        #self.images = np.load(loc+'testtest'+'_features.npy', mmap_mode= 'r')
...
#         image_id = self.imgid[index]
        ...
        vocab = self.vocab
        idx = torch.tensor(self.ids[img_id])
        # Convert caption (string) to word ids.
        tokens = nltk.tokenize.word_tokenize(
            str(caption).lower())
        tokens_language = nltk.tokenize.word_tokenize(
            str(caption).replace('b\'', '').replace('-', ' ').lower())
        ...
        language = ['<start>']+ tokens_language + ['<end>']
        language2 = []
        for ii, w in enumerate(language):
            if w in self.vocab_int:
                language2.append(torch.tensor(vectors[word2idx[w]]))

        language2 = torch.stack(language2, 0)

        language_all = torch.zeros([87, 300])

        language_all[:language2.size(0),:] = language2
        ...
        return image, target, index, img_id, caption_label, caption_mask, idx, boxes, image_id, language_all
...
def collate_fn(data):
    ...
    images, captions, ids, img_ids, caption_labels, caption_masks, idx, boxes, image_id, language = zip(*data)
    ...
    boxes = 1
    #image_id = torch.stack(image_id, 0)
    ...
    language2 = torch.stack(language, 0)
    #lengths2 = [len(cap) for cap in language]
    #language2 = torch.zeros(len(language), max(lengths2), 300).float()
    #for ii, lan in enumerate(language):
    #    end = lengths2[ii]
    #    language2[ii, :end, :] = lan[:end]
    return images, targets, lengths, ids, caption_labels_, caption_masks_, idx, boxes, image_id, language2


def get_loader_single(data_name, split, root, json, vocab, transform,
                      batch_size=128, shuffle=True,
                      num_workers=8, ids=None, collate_fn=collate_fn):
